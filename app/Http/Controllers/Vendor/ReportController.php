...
class ReportController extends Controller
{
    public function dashboard(Request $request)
    {
        ...
        return view('vendor-views.report.index', compact('data', 'earning', 'commission', 'params'));
    }

    public function restaurant_data()
    {
        $new_pending_takeway_order = DB::table('orders')->where(['checked' => 0])->where('restaurant_id', Helpers::get_restaurant_id())->where('order_status','pending')->where('order_type', 'take_away')->count();
        $new_confirmed_order = DB::table('orders')->where(['checked' => 0])->where('restaurant_id', Helpers::get_restaurant_id())->where('order_status','confirmed')->count();
        
        return response()->json([
            'success' => 1,
            'data' => ['new_pending_takeway_order' => $new_pending_takeway_order, 'new_confirmed_order' => $new_confirmed_order]
        ]);
    }
    ...
    public function dashboard_order_stats_data()
    {
        ...
        })->where(['order_status' => 'confirmed', 'restaurant_id' => Helpers::get_restaurant_id()])->count();
        ...
        $scheduled = Order::when($today, function ($query) {
            ...
            return $query->whereMonth('created_at', Carbon::now());
        })->Scheduled()->where(['restaurant_id' => Helpers::get_restaurant_id()])->count();
